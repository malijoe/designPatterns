...
func NewUser2(fullName string) *User2 {
    ...
    for _, part := range parts {
        result.names = append(result.names, getOrAdd(part))
    }
...
func UserNames() {
    ...
    fmt.Println("Memory taken by users:", len([]byte(john.FullName)) + len([]byte(jane.FullName)) + len([]byte(alsoJane.FullName)))
    ...
    fmt.Println("Memory take by users2:", totalMem)
